...
function checkRejections (rejections, config, sendToGEQ) {
		...
		if(typeof normalRejections.count !== 'number')
			throw new Error(MESSAGES.REASON_CORRUPTED_NORMAL_REJECTIONS);

		if(normalRejections.count > 1 || totalRejections > 4)
			throw new Error(MESSAGES.REASON_REJECTED_TOO_MANY_TIMES(normalRejections.count, totalRejections));
...
function sentTo(channel, message, originalRoute, exchangeName, reason, errorString) {
	try{
		...
		.catch(e => logError(errorString, e, JSON.stringify(message)));
	}catch(e){
