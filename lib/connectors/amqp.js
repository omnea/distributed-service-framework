...
module.exports = class AMQPBuilder extends EventEmitter {
	constructor(amqp, AmqpChannel, predefinedOptions, options) {
		super();
		
		this.options = this._mergeConfs(predefinedOptions, options);
	...
	channel () {
		...
		.then(_ => this.channels.add(channel))
		.then(_ => channel);
	}
	
	_createAMQPUrl (options) {
		return 'amqp://' + options.user + ':' + options.pass + '@' + options.url +':' + options.port;
	}
	
	_mergeConfs (predefinedOptions, conf) {
		if(!conf) conf = {};
		
		return Object.assign({}, predefinedOptions, conf);
	}
};
