...
module.exports = class AMQPBuilder extends EventEmitter{
	...
	unbindQueue(queueName, exchangeSource, route, options) {
		return this.channel.bindQueue(queueName, exchangeSource, route, options)
		.then(() => debug('Queue ' + queueName + ' unbinded to ' + exchangeSource + ':' + route, this.id, this.connectionId))
		.catch(err => {
			var errorData = this._createErrorBinding(queueName, exchangeSource, route, options, err);
			error(errorData);
	...
	emit(exchange, route, content) {
		return this.channel.publish(exchange, route, content)
		.then(() => debug('Message published'));
	}

	_createErrorBinding(queueName, exchangeSource, route, options) {
		return {
			queueName, 
			exchangeSource, 
			route, 
