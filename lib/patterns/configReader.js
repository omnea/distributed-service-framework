...
module.exports = class ConfigReader {
	...
	getGlobalErrorQueueName() {
		var queueName = this.get('globalErrorQueue.name');
	...
	getGlobalErrorExchangeName() {
		var name = this.get('globalErrorExchange.name');
	...
	getGlobalErrorQueueOptions() {
		var queueName = this.get('globalErrorQueue.options');
