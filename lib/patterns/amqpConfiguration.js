...
module.exports = class AmqpConfiguration {
	...
	_declareExchanges() {
		return Promise.all([
			this.declaringChannel.exchange(this.config.getExchangeName('service'), this.config.getExchangeType('service'), this.config.getExchangeOptions('service'))
			.then(exchange => this.exchange = exchange),
			this.declaringChannel.exchange(this.config.getExchangeName('delay'), this.config.getExchangeType('delay'), this.config.getExchangeOptions('delay'))
			.then(exchange => this.exchange = exchange)
		]);
	}
